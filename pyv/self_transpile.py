...
def _render_cli_module(transpiler: Any) -> str:
    ...
    return "\n".join(
        [
            ...
            transpiler.indent("return value"),
            "}",
            "",
            "fn rust_print_from_args(args string) string {",
            transpiler.indent("parts := args.split(',').map(it.trim_space())"),
            transpiler.indent("if parts.len == 1 {"),
            transpiler.indent("part := parts[0]", 2),
            transpiler.indent(quote_check.replace("inner", "part"), 2),
            transpiler.indent("text := part[1..part.len - 1]", 3),
            transpiler.indent(
                "return '    println!(\"' + rust_string_literal(text) + '\");'",
                3,
            ),
            transpiler.indent("}", 2),
            transpiler.indent("return '    println!(\"{}\", ' + part + ');'", 2),
            transpiler.indent("}"),
            transpiler.indent("mut fmt := []string{}"),
            transpiler.indent("mut exprs := []string{}"),
            transpiler.indent("for part in parts {"),
            transpiler.indent("fmt << '{}'", 2),
            transpiler.indent("exprs << part", 2),
            transpiler.indent("}"),
            transpiler.indent(
            ...
            transpiler.indent("_ := parse_python_source_json(source)"),
            transpiler.indent("trimmed := source.trim_space()"),
            transpiler.indent(
                "if trimmed.starts_with('print(') && trimmed.ends_with(')') {"
            ),
            transpiler.indent("inner := trimmed[6..trimmed.len - 1].trim_space()", 2),
            transpiler.indent(quote_check, 2),
            transpiler.indent("text := inner[1..inner.len - 1]", 3),
            transpiler.indent(
            ...
            transpiler.indent("for line in source.split_into_lines() {"),
            transpiler.indent("stripped := line.trim_space()", 2),
            transpiler.indent(
            ...
            transpiler.indent("}", 2),
            transpiler.indent("}"),
            transpiler.indent("if body.len > 0 {"),
            transpiler.indent(
                "return 'fn main() {\\n' + body.join('\\n') + '\\n}\\n'",
                2,
            ),
            ...
            transpiler.indent("return pyast.parse_module_json(source) or { '{}' }"),
            "}",
            "",
            "pub fn run_cli() {",
            ...
            transpiler.indent("rust := rust_from_source(source)"),
