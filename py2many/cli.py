...
def _process_dir(
    ...
    if settings.ext == ".v":
        _write_v_project_manifest(outdir)
...
def _write_v_project_manifest(outdir):
    manifest = outdir / "v.mod"
    if not manifest.exists():
        manifest.write_text(
            "Module {\n"
            "    name: 'py2many_generated'\n"
            "    description: 'Generated by py2many'\n"
            "    version: '0.0.0'\n"
            "    license: 'MIT'\n"
            "    dependencies: ['py2many.v_ast']\n"
            "}\n"
        )
