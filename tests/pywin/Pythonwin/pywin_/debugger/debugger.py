...
# >>> win32com_.gen_py.debugger.GetDebugger().run("command")
...
from win32com_.gen_py.mfc import dialog, object, afxres, window
from win32com_.gen_py.framework import app, interact, editor, scriptutils
from win32com_.gen_py.framework.editor.color.coloreditor import MARKER_CURRENT, MARKER_BREAKPOINT
from win32com_.gen_py.tools import browser, hierlist
import commctrl
import traceback
...
class DebuggerWindow(window.Wnd):
    ...
    def OnDestroy(self, msg):
        ...
        win32com_.gen_py.framework.app.SaveWindowSize("Debugger Windows\\" + self.title, newSize)
...
def PrepareControlBars(frame):
    ...
    for id, klass, float in DebuggerDialogInfos:
        ...
        if exists:
            continue
        bar = win32com_.gen_py.docking.DockingBar.DockingBar()
