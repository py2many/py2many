...
from . import app
import traceback
from win32com_.gen_py.mfc import afxres, dialog
import commctrl
...
def _SetupSharedMenu_(self):
    ...
    from win32com_.gen_py.framework import toolmenu

    toolmenu.SetToolsMenu(sharedMenu)
    from win32com_.gen_py.framework import help

    help.SetHelpMenuOtherHelp(sharedMenu)


from win32com_.gen_py.mfc import docview
...
class MainFrame(app.MainFrame):
    def OnCreate(self, createStruct):
        ...
        from win32com_.gen_py.debugger.debugger import PrepareControlBars
    ...
    def OnClose(self):
        try:
            ...
            if (
                win32com_.gen_py.debugger.currentDebugger is not None
                and win32com_.gen_py.debugger.currentDebugger.pumping
            ):
                try:
                    win32com_.gen_py.debugger.currentDebugger.close(1)
                except:
                    traceback.print_exc()
                return
...
class InteractivePythonApp(app.CApp):
    ...
    def ProcessArgs(self, args, dde=None):
        ...
        while i < len(args):
            ...
            if argType in ["/nodde", "/new", "-nodde", "-new"]:
                ...
                if dde:
                    dde.Exec(
                        "from win32com_.gen_py.framework import scriptutils\n"
                ...
                if not os.path.isfile(fname):
                    ...
                    # win32com_.gen_py.scintilla.document.CScintillaDocument.OnOpenDocument)
                ...
                if dde:
                    dde.Exec(
                        "from win32com_.gen_py.framework import scriptutils;scriptutils.RunScript(%r, %r, 1)"
                        % (par, " ".join(args[i + 1 :]))
                    )
                else:
                    from . import scriptutils
                ...
                if dde:
                    dde.Exec(
                        "from win32com_.gen_py.framework import scriptutils;scriptutils.RunScript(%r, %r, 0)"
                        % (par, " ".join(args[i + 1 :]))
                    )
                else:
                    from . import scriptutils
    ...
    def LoadSystemModules(self):
        self.DoLoadModules("win32com_.gen_py.framework.editor,win32com_.gen_py.framework.stdin")
    ...
    def OnViewOptions(self, id, code):
        ...
        from win32com_.gen_py.dialogs import ideoptions
        ...
        try:
            from win32com_.gen_py.debugger import configui
        except ImportError:
            configui = None
    ...
    def OnFileSaveAll(self, id, code):
        ...
        from win32com_.gen_py.framework.editor import editorTemplate
