...
class HLICLBRItem(hierlist.HierListItem):
    ...
    def TakeDefaultAction(self):
        if self.file:
            win32com_.gen_py.framework.scriptutils.JumpToDocument(
...
class HLIModuleItem(hierlist.HierListItem):
    ...
    def GetSubList(self):
        mod, path = win32com_.gen_py.framework.scriptutils.GetPackageModuleName(self.path)
...
class HLIDirectoryItem(hierlist.HierListItem):
    ...
    def GetSubList(self):
        ...
        if (
            os.path.split(self.path)[1] == "win32com_"
        ):  # Complete and utter hack for win32com_.
            try:
