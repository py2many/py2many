...
BOOL LoadGatewayModule(PyObject **ppModule)
{
    ...
    pPyModule = PyImport_ImportModule("win32com_.server.policy");
    if (!pPyModule) {
        PyCom_LoggerException(NULL, L"PythonCOM Server - The 'win32com_.server.policy' module could not be loaded.");
