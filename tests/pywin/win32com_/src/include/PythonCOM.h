...
// If logProvider is NULL, we attempt to fetch "win32com_.logger".
...
// NOTE: By default, win32com_ does *not* provide a logger, so default is that
...
class PYCOM_EXPORT PythonOleArgHelper {
   public:
    ...
    PyObject *m_pyVariant;  // if non-null, a win32com_.client.VARIANT
    union {
        void *m_pValueHolder;
        short m_sBuf;
        long m_lBuf;
