...
them = (lower(ver_desc(other) || ""), lower(desc(other) || ""), major(other), minor(other))
return me < them
end

function __eq__(self::AbstractTypelibSpec, other)
return lower(self.ver_desc || "") == lower(ver_desc(other) || "") && lower(self.desc || "") == lower(desc(other) || "") && self.major == major(other) && self.minor == minor(other)
end
