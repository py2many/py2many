...
is_readonly = hasattr(win32com, "__loader__") && hasattr(win32com.__loader__, "archive")
is_zip = hasattr(win32com, "__loader__") && hasattr(win32com.__loader__, "archive")
    ...
    try
        ...
        if module_ != nothing && bValidateFile
            ...
            try
                ...
                if !(suf)
                    try
                        typLibPath = Vector{UInt8}(typLibPath)
                    catch exn
                        if exn isa AttributeError
    ...
    for info in infos
        ...
        try
            ...
            @printf(
                ...
                (info, exc_info(sys)[1], exc_info(sys)[2])
            )
        end
    end
    if verbose && length(infos)
