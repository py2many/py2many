...
import ext_modules.win32con as win32con
...
function GetSubList(self::HLI_IEnumMoniker)::Vector
    ...
    for mon in Enumerator(util, self.myobject)
        append(ret, HLI_IMoniker(mon, GetDisplayName(mon, ctx, nothing)))
    end
    return ret
end
...
function GetSubList(self::HLI_IMoniker)::Vector
    ret = []
    append(ret, MakeHLI(browser, Hash(self.myobject), "Hash Value"))
    subenum = Enum(self.myobject, 1)
    append(ret, HLI_IEnumMoniker(subenum, "Sub Monikers"))
    return ret
end
...
function GetSubList(self::HLIHeadingCategory)::Vector
    ...
    try
        for (catid, lcid, desc) in enum
            append(ret, HLICategory((catid, lcid, desc)))
        end
    catch exn
...
function TakeDefaultAction(self::HLIHelpFile)
    fname, ctx = self.myobject
    if ctx
        cmd = HELP_CONTEXT(win32con)
    else
        cmd = HELP_FINDER(win32con)
    end
...
function GetSubList(self::HLIRegisteredTypeLibrary)::Vector
            ...
            elseif subKey == "Flags"
                ...
                try
                    ...
                    while true
                        ...
                        append(collected, (lcid, platform, fname))
                        lcidnum = lcidnum + 1
                    end
    ...
    ret = []
    append(ret, HLICLSID(clsidstr))
    for (lcid, platform, fname) in collected
        extraDescs = []
        if platform != "win32"
            append(extraDescs, platform)
        end
        if lcid
            append(extraDescs, "locale=%s" % lcid)
        end
        extraDesc = ""
        if extraDescs
            extraDesc = " (%s)" % x -> join(x, extraDescs)
        end
        append(ret, HLITypeLib(fname, "Type Library" * extraDesc))
    end
    sort(ret)
    return ret
end
...
function GetSubList(self::HLITypeLibEntry)::Vector
    ...
    ret = []
    if doc
        append(ret, HLIDocString(browser, doc, "Doc"))
    end
    if helpFile
        append(ret, HLIHelpFile((helpFile, ctx)))
    end
    return ret
end
...
function GetSubList(self::HLITypeLibEnum)::Vector
    ...
    for j = 0:attr[8]-1
        ...
        append(ret, MakeHLI(browser, vdesc[2], name))
    end
    return ret
end
...
function GetSubList(self::HLITypeLibProperty)::Vector
    ...
    if length(names) > 1
        append(ret, MakeHLI(browser, names[2:end], "Named Params"))
    end
    vd = GetVarDesc(typeinfo, index)
    append(ret, MakeHLI(browser, self.id, "Dispatch ID"))
    append(ret, MakeHLI(browser, vd[2], "Value"))
    append(ret, MakeHLI(browser, vd[3], "Elem Desc"))
    append(ret, MakeHLI(browser, vd[4], "Var Flags"))
    append(ret, MakeHLI(browser, vd[5], "Var Kind"))
    return ret
end
...
function GetSubList(self::HLITypeLibFunction)::Vector
    ...
    append(ret, MakeHLI(browser, self.id, "Dispatch ID"))
    if length(names) > 1
        append(ret, MakeHLI(browser, x -> join(x, names[2:end]), "Named Params"))
    end
    fd = GetFuncDesc(typeinfo, index)
    if fd[2]
        append(ret, MakeHLI(browser, fd[2], "Possible result values"))
    end
    if fd[9]
        ...
        append(ret, MakeHLI(browser, val, "Return Type"))
    end
    for argDesc in fd[3]
        ...
        append(ret, MakeHLI(browser, val, "Argument"))
    end
    try
    ...
    append(ret, MakeHLI(browser, fkind, "Function Kind"))
    try
    ...
    append(ret, MakeHLI(browser, ikind, "Invoke Kind"))
    append(ret, MakeHLI(browser, fd[7], "Number Optional Params"))
    flagDescs = []
    for (flag, desc) in self.funcflags
        if flag & fd[10]
            append(flagDescs, desc)
        end
    end
    if flagDescs
        append(ret, MakeHLI(browser, x -> join(x, flagDescs), "Function Flags"))
    end
    return ret
end
...
function GetSubList(self::HLITypeLib)::Vector
    ret = []
    append(ret, MakeHLI(browser, self.myobject, "Filename"))
    try
    ...
    for i = 0:GetTypeInfoCount(tlb)-1
        try
            append(ret, HLITypeKinds[GetTypeInfoType(tlb, i)][1]((tlb, i)))
        catch exn
            if exn isa com_error(pythoncom)
                append(ret, MakeHLI(browser, "The type info can not be loaded!"))
            end
        end
    end
    sort(ret)
...
function GetSubList(self::HLIHeadingRegisterdTypeLibs)::Vector
    ...
    try
        ...
        while true
            ...
            if name != nothing
                append(ret, HLIRegisteredTypeLibrary((keyName, versionStr), name))
            end
            num = num + 1
        end
    finally
