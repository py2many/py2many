...
return CoCreateInstance(pythoncom, CLSID_StdComponentCategoriesMgr(pythoncom), nothing, CLSCTX_INPROC_SERVER(pythoncom), IID_ICatRegister(pythoncom))
end
...
path = win32com.server.__path__[1]
...
if exn isa os.error
throw(RuntimeError("Can not locate the Python module \'win32com.server.%s\'" % baseName))
end
end
end
end
...
try
moduleName = splitext(os.path, FindFiles(win32api, append!([PROGRAM_FILE], ARGS)[1])[1][9])[1]
catch exn
...
cwd = getcwd(os)
write(batf, )
write(batf, "set PYTHONPATH=%s", get(os.environ, "PYTHONPATH", ""))
write(batf, )
write(batf, "cd \"%s\"", getcwd(os))
write(batf, "%s %s > \"%s\" 2>&1", (GetShortPathName(win32api, exe_to_run), new_params, outfile))
finally
close(batf)
end
...
if exit_code
@printf("Error: registration failed (exit code %s).", exit_code)
end
...
std::fs::remove_file(f)
 ...
 let exc = exn
if exc isa os.error
@printf("Failed to remove tempfile \'%s\': %s", (f, exc))
end
end
end
end
...
unregisterInfo = "--unregister_info" in append!([PROGRAM_FILE], ARGS)
unregister = "--unregister" in append!([PROGRAM_FILE], ARGS)
flags["quiet"] = get(flags, "quiet", 0) || "--quiet" in append!([PROGRAM_FILE], ARGS)
flags["debug"] = get(flags, "debug", 0) || "--debug" in append!([PROGRAM_FILE], ARGS)
flags["unattended"] = get(flags, "unattended", 0) || "--unattended" in append!([PROGRAM_FILE], ARGS)
if unregisterInfo
...
if flags["unattended"] || winerror(exc) != winerror.ERROR_ACCESS_DENIED || getwindowsversion(sys)[1] < 5
error()
end
...
if !frozen(pythoncom)
try
RegQueryValue(win32api, win32con.HKEY_CLASSES_ROOT, "Component Categories\\%s" % CATID_PythonCOMServer)
catch exn
if exn isa error(win32api)
try
