...
S_OK = 0
IDispatchType = TypeIIDs(pythoncom)[IID_IDispatch(pythoncom) + 1]
...
IUnknownType = TypeIIDs(pythoncom)[IID_IUnknown(pythoncom) + 1]
...
try
dispatcher = RegQueryValue(win32api, win32con.HKEY_CLASSES_ROOT, regDispatcher % clsid)
if dispatcher
...
self._query_interface_ = _query_interface_(ob)
end
if hasattr(ob, "_invoke_")
self._invoke_ = _invoke_(ob)
end
if hasattr(ob, "_invokeex_")
self._invokeex_ = _invokeex_(ob)
end
if hasattr(ob, "_getidsofnames_")
self._getidsofnames_ = _getidsofnames_(ob)
end
if hasattr(ob, "_getdispid_")
self._getdispid_ = _getdispid_(ob)
end
if hasattr(ob, "_com_interfaces_")
self._com_interfaces_ = []
for i in _com_interfaces_(ob)
if type_(i) != IIDType(pywintypes)
if i[1] != "{"
i = InterfaceNames(pythoncom)[i + 1]
else
...
function _Invoke_(self::BasicWrapPolicy, dispid, lcid, wFlags, args)::Tuple
...
try
dispid = self._name_to_dispid_[lower(dispid) + 1]
catch exn
if exn isa KeyError
...
function _getdispid_(self::BasicWrapPolicy, name, fdex)
try
return self._name_to_dispid_[lower(name) + 1]
catch exn
if exn isa KeyError
...
self._dispid_to_func_ = _dispid_to_func_(ob)
else
self._dispid_to_func_ = Dict()
end
if hasattr(ob, "_dispid_to_get_")
self._dispid_to_get_ = _dispid_to_get_(ob)
else
self._dispid_to_get_ = Dict()
end
if hasattr(ob, "_dispid_to_put_")
self._dispid_to_put_ = _dispid_to_put_(ob)
else
self._dispid_to_put_ = Dict()
end
end
...
elseif dispid in self._dispid_to_get_
...
tlb_guid = getattr(ob, "_typelib_guid_", nothing)
if tlb_guid != nothing
tlb_major, tlb_minor = getattr(ob, "_typelib_version_", (1, 0))
tlb_lcid = getattr(ob, "_typelib_lcid_", 0)
interfaces = [i for i in getattr(ob, "_com_interfaces_", []) if type_(i) != IIDType(pywintypes) && !startswith(i, "{") ]
universal_data = RegisterInterfaces(universal, tlb_guid, tlb_lcid, tlb_major, tlb_minor, interfaces)
else
universal_data = []
end
_wrap_(MappedWrapPolicy, self)
if !hasattr(ob, "_public_methods_") && !hasattr(ob, "_typelib_guid_")
...
elseif invkind in (DISPATCH_PROPERTYPUT, DISPATCH_PROPERTYPUTREF)
...
readonly = _readonly_attrs_(ob)
else
readonly = []
end
for name in _public_attrs_(ob)
...
tlb_guid = getattr(self._obj_, "_typelib_guid_", nothing)
if tlb_guid === nothing
return []
end
...
function _GetTypeInfo_(self::DesignatedWrapPolicy, index, lcid)::Tuple
if self._typeinfos_ === nothing
self._typeinfos_ = _build_typeinfos_(self)
end
...
if type_(retob) == MethodType(types)
retob = retob(args...)
end
return retob
end
...
if type_(getattr(self._obj_, name, nothing)) == MethodType(types) && type_(getattr(self._obj_, "Set" + name, nothing)) == MethodType(types)
...
function _transform_args_(self::EventHandlerPolicy, args, kwArgs, dispid, lcid, wFlags, serviceProvider)::Tuple
ret = []
for arg in args
arg_type = type_(arg)
if arg_type == IDispatchType
arg = Dispatch(win32com.client, arg)
elseif arg_type == IUnknownType
try
arg = Dispatch(win32com.client, QueryInterface(arg, IID_IDispatch(pythoncom)))
catch exn
...
_obj_::Any
_next_dynamic_::Any
_dyn_dispid_to_name_::Any
_name_to_dispid_::Any
end
function _wrap_(self::DynamicPolicy, object)
_wrap_(BasicWrapPolicy, self)
if !hasattr(self._obj_, "_dynamic_")
...
function _getdispid_(self::DynamicPolicy, name, fdex)
lname = lower(name)
try
...
function _invoke_(self::DynamicPolicy, dispid, lcid, wFlags, args)::Tuple
return (S_OK, -1, _invokeex_(self, dispid, lcid, wFlags, args, nothing, nothing))
end
