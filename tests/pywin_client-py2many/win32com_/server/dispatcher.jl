...
policy::Any
                    ...
                    DispatcherBase(logger::Any = getattr(win32com, "logger", nothing), policy::Any = policyClass(object)) =
                        new(logger, policy)
end
...
debug = get_option(GetDebugger(pywin.debugger), OPT_STOP_EXCEPTIONS(pywin.debugger.dbgcon))
end
debug = 1
end
if debug != 0
try
post_mortem(pywin.debugger, tb, typ, val)
catch exn
current_exceptions() != [] ? current_exceptions()[end] : nothing
end
end
