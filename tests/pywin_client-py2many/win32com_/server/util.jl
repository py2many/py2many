...
iid::Any
                    ...
                    ListEnumerator(index::Any, _iid_::Any = iid, _list_::Any = data, _public_methods_::Vector{String} = ["Next", "Skip", "Reset", "Clone"], iid::Any = IID_IEnumVARIANT(pythoncom)) =
...
function NewEnum(seq, cls = ListEnumerator, iid = IID_IEnumVARIANT(pythoncom), usePolicy = nothing, useDispatcher = nothing)
...
readOnly::Int64
            ...
            Collection(data = nothing, readOnly = 0, _public_methods_::Vector{String} = ["Item", "Count", "Add", "Remove", "Insert"], _value_ = Item) = begin
                if data === nothing
data = []
end
if readOnly
self._public_methods_ = ["Item", "Count"]
                ...
                new(data , readOnly , _public_methods_, _value_ )
            end
end
...
#= Creates a new COM collection object
    ...
    (the default is @Collection@).
     =#
return WrapObject(pythoncom, DefaultPolicy(policy, cls(seq)), IID_IDispatch(pythoncom), IID_IDispatch(pythoncom))
end
...
_public_methods_::Vector{String}
                    ...
                    FileStream(file::Any, _com_interfaces_::Vector = [IID_IStream(pythoncom)], _public_methods_::Vector{String} = ["Read", "Write", "Clone", "CopyTo", "Seek"]) =
...
function CopyTo(self::FileStream, dest, cb)::Tuple
