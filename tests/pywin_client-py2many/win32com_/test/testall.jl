...
using win32com.test: pippo_server
import string
import getopt
try
this_file = __file__
...
import win32com
win32com.__path__[1] = win32com_src_dir
abstract type AbstractPyCOMTest <: AbstractTestCase end
abstract type AbstractPippoTest <: AbstractTestCase end

import win32com.client
using win32com.test.util: CheckClean, TestCase, CapturingFunctionTestCase, ShellTestCase, TestLoader, TestRunner, RegisterPythonServer
verbosity = 1
...
if isdir(os.path, win32com.__gen_path__)
if verbosity > 1
@printf("Deleting files from %s", win32com.__gen_path__)
end
rmtree(shutil, win32com.__gen_path__)
end
__init__(win32com.client.gencache)
end
...
unittest_other_modules = [split("win32com.directsound.test.ds_test\n        "), [], [], []]
...
func = func_name === nothing ? (nothing) : (getattr(mod, func_name))
return (mod, func)
end

function make_test_suite(test_level = 1)::Tuple
...
@printf("Executing level %d tests - %d test cases will be run", (testLevel, countTestCases(suite)))
