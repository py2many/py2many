...
function BeginThreadsSimpleMarshal(self::ThreadInterpCase, numThreads)::Tuple
...
push!(events, hEvent)
interpStream = CoMarshalInterThreadInterfaceInStream(pythoncom, IID_IDispatch(pythoncom), _oleobj_(interp))
...
function BeginThreadsFastMarshal(self::ThreadInterpCase, numThreads)::Tuple
...
interp = Dispatch(win32com.client, "Python.Interpreter")
if freeThreaded != 0
interp = CoMarshalInterThreadInterfaceInStream(pythoncom, IID_IDispatch(pythoncom), _oleobj_(interp))
end
events = []
threads = []
for i in 0:numThreads - 1
