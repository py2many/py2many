...
assertRaisesCOM_HRESULT(win32com.test.util, self, winerror.DISP_E_TYPEMISMATCH, interp.Eval, 2)
end

function testInproc(self::InterpCase)
interp = Dispatch(win32com.client.dynamic, "Python.Interpreter", CLSCTX_INPROC(pythoncom))
_testInterp(self, interp)
end

function testLocalServer(self::InterpCase)
interp = Dispatch(win32com.client.dynamic, "Python.Interpreter", CLSCTX_LOCAL_SERVER(pythoncom))
_testInterp(self, interp)
end
