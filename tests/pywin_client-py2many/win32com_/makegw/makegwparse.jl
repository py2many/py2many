...
gatewayMode::Int64
                    ...
                    ArgFormatter(arg::Any, builtinIndirection::Any, declaredIndirection::Any, gatewayMode::Int64 = 0) =
...
function GetInterfaceCppObjectInfo(self::ArgFormatter)::Tuple
...
function GetInterfaceCppObjectInfo(self::ArgFormatterIID)::Tuple
...
throw(error_not_supported("The type \'%s\' (%s) is unknown." % (type_(arg), name(arg))))
end
end
end
...
regex::Any
                    ...
                    Argument(good_interface_names::Any, indirectionLevel::Any, name::Any, raw_type::Any, type::Any, unc_type::Any, arrayDecl::Int64 = 0, const::Int64 = 0, inout::Any = nothing, regex::Any = compile(re, "/\\* \\[([^\\]]*.*?)] \\*/[ \\t](.*[* ]+)(\\w+)(\\[ *])?[\\),]")) =
                        new(good_interface_names, indirectionLevel, name, raw_type, type, unc_type, arrayDecl, const, inout, regex)
end
...
if VERBOSE != 0
@printf("\t   Arg %s of type %s%s (%s)", (self.name, self.type, "*"*self.indirectionLevel, self.inout))
end
end
...
regex::Any
                    ...
                    Method(good_interface_names::Any, result::Any, args::Vector = [], name::Any = nothing, regex::Any = compile(re, "virtual (/\\*.*?\\*/ )?(.*?) (.*?) (.*?)\\(\\w?")) =
                        new(good_interface_names, result, args, name, regex)
end
...
@printf("Method %s - Only HRESULT return types are supported.", self.name)
end
@printf("\t Method %s %s(", (self.result, self.name))
end
while true
...
regex::Any
            ...
            Interface(mo, base = group(mo, 3), methods::Vector = [], name = group(mo, 2), regex = compile(re, "(interface|) ([^ ]*) : public (.*)\$")) = begin
                if VERBOSE
@printf("Interface %s : public %s", (self.name, self.base))
end
                new(mo, base , methods, name , regex )
            end
end
...
@printf("%d\n%s\n%s\n%s\n%s", (res, group(r, 1), group(r, 2), group(r, 3), group(r, 4)))
end
end
