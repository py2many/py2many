...
@printf("Table %s - Fields: %d, Attributes:%d", (tab.Name, length(tab.Fields), tab.Attributes))
if bDeep
...
for field in fields
@printf("  %s, size=%d, reqd=%d, type=%d, defVal=%s", (field.Name, field.Size, field.Required, field.Type, string(field.DefaultValue)))
end
end

function DumpRelations(db, bDeep = 1)
for relation in db.Relations
@printf("Relation %s - %s->%s", (relation.Name, relation.Table, relation.ForeignTable))
end
end
