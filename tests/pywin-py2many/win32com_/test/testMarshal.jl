...
push!(events, hEvent)
interpStream = CoMarshalInterThreadInterfaceInStream(pythoncom, pythoncom.IID_IDispatch, interp._oleobj_)
...
interp = Dispatch(win32com_.client, "Python.Interpreter")
if freeThreaded != 0
interp = CoMarshalInterThreadInterfaceInStream(pythoncom, pythoncom.IID_IDispatch, interp._oleobj_)
end
events = []
threads = []
for i in 0:numThreads - 1
