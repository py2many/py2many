...
item = VTableItem(win32com_.client.build, type_info, attr, GetDocumentation(type_info, -1))
...
is_dispatch = hasfield(sub_mod, name + "_vtables_dispatch_"): getfield(sub_mod, name + "_vtables_dispatch_" ? nothing
method_defs = hasfield(sub_mod, name + "_vtables_"): getfield(sub_mod, name + "_vtables_" ? nothing
...
ob = hasfield(ob, "policy"): getfield(ob, "policy" ? ob
