...
tlb_guid = hasfield(ob, "_typelib_guid_"): getfield(ob, "_typelib_guid_" ? nothing
if tlb_guid != nothing
tlb_major, tlb_minor = hasfield(ob, "_typelib_version_"): getfield(ob, "_typelib_version_" ? (1, 0)
tlb_lcid = hasfield(ob, "_typelib_lcid_"): getfield(ob, "_typelib_lcid_" ? 0
interfaces = [i for i in hasfield(ob, "_com_interfaces_"): getfield(ob, "_com_interfaces_" ? [] if type_(i) != pywintypes.IIDType && !startswith(i, "{") ]
...
for name in hasfield(ob, "_public_methods_"): getfield(ob, "_public_methods_" ? []
dispid = get(self._name_to_dispid_, lower(name))
if dispid === nothing
dispid = next_dispid
self._name_to_dispid_[lower(name) + 1] = dispid
...
tlb_guid = hasfield(self._obj_, "_typelib_guid_"): getfield(self._obj_, "_typelib_guid_" ? nothing
if tlb_guid === nothing
return []
end
tlb_major, tlb_minor = hasfield(self._obj_, "_typelib_version_"): getfield(self._obj_, "_typelib_version_" ? (1, 0)
...
if type_(hasfield(self._obj_, name): getfield(self._obj_, name ? nothing) == types.MethodType && type_(hasfield(self._obj_, "Set" + name): getfield(self._obj_, "Set" + name ? nothing) == types.MethodType
...
try
arg = Dispatch(win32com_.client, QueryInterface(arg, pythoncom.IID_IDispatch))
catch exn
