...
return CoCreateInstance(pythoncom, pythoncom.CLSID_StdComponentCategoriesMgr, nothing, pythoncom.CLSCTX_INPROC_SERVER, pythoncom.IID_ICatRegister)
end
...
write(batf, "$(g)\"")
write(batf, "$(None) $(new_params)\" 2>&1")
finally
close(batf)
end
...
if flags["unattended"] || exc.winerror != winerror.ERROR_ACCESS_DENIED || getwindowsversion(sys)[1] < 5
error()
end
