...
is_readonly = hasattr(win32com_, "__loader__") && hasattr(win32com_.__loader__, "archive")
is_zip = hasattr(win32com_, "__loader__") && hasattr(win32com_.__loader__, "archive")
...
#= Generate support for a type library.
    ...
    progressInstance -- Instance to use as progress indicator, or None to
     ...
     =#
GenerateFromTypeLibSpec(makepy, (typelibCLSID, lcid, major, minor), progressInstance, bForDemand, bBuildHidden)
...
#= Generate support for a type library.
    ...
    progressInstance -- Instance to use as progress indicator, or None to
     ...
     =#
try
GenerateFromTypeLibSpec(makepy, typelib_ob, progressInstance, bForDemandDefault, bBuildHidden)
catch exn
if exn isa pywintypes.com_error
return nothing
end
...
if exn isa KeyError
@printf("ForgetAboutTypelibInterface:: Warning - type library with info %s is not being remembered!", (info,))
end
end
...
module_ = nothing
try
tlbAttr = GetLibAttr(LoadRegTypeLib(pythoncom, typelibCLSID, major, minor, lcid))
...
@assert(!(is_readonly))
try
typLibPath = QueryPathOfRegTypeLib(pythoncom, typelibCLSID, major, minor, lcid)
if typLibPath[end] == " "
typLibPath = typLibPath[begin:-1]
end
suf = hasfield(os.path, "supports_unicode_filenames"): getfield(os.path, "supports_unicode_filenames" ? 0
if !(suf)
try
...
tlbAttributes = GetLibAttr(LoadRegTypeLib(pythoncom, typelibCLSID, major, minor, lcid))
catch exn
if exn isa pythoncom.com_error
bValidateFile = 0
end
...
@printf("Could not add module %s - %s: %s", (info, exc_info(sys)[1], exc_info(sys)[2]))
end
end
if verbose && length(infos)
println("Done.")
