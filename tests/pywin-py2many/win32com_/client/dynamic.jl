...
IDispatch = CoCreateInstance(pythoncom, IDispatch, nothing, clsctx, pythoncom.IID_IDispatch)
end
end
else
IDispatch = hasfield(IDispatch, "_oleobj_"): getfield(IDispatch, "_oleobj_" ? IDispatch
end
return IDispatch
end
...
@printf("\t%s = 0x%x - %s", (prop, entry.dispid, repr(entry)))
end
println("Get Props:")
