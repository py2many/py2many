...
try
tlb = LoadRegTypeLib(pythoncom, tlbSpec.clsid, tlbSpec.major, tlbSpec.minor, tlbSpec.lcid)
catch exn
...
println(desc)
@printf(" %s, lcid=%s, major=%s, minor=%s", (tlbSpec.clsid, tlbSpec.lcid, tlbSpec.major, tlbSpec.minor))
println(" >>> # Use these commands in Python code to auto generate .py support")
println(" >>> from win32com_.client import gencache")
@printf(" >>> gencache.EnsureModule(\'%s\', %s, %s, %s)", (tlbSpec.clsid, tlbSpec.lcid, tlbSpec.major, tlbSpec.minor))
end
end
...
for spec in tlbs
if spec.dll === nothing
tlb = LoadRegTypeLib(pythoncom, spec.clsid, spec.major, spec.minor, spec.lcid)
else
...
tlb = LoadRegTypeLib(pythoncom, typelibInfo.clsid, typelibInfo.major, typelibInfo.minor, typelibInfo.lcid)
else
...
if file === nothing
this_name = GetGeneratedFileName(gencache, info.clsid, info.lcid, info.major, info.minor)
full_name = join
if bForDemand
try
...
progress = progressInstance
for (typelib, info) in typelibs
dir_name = GetGeneratedFileName(gencache, info.clsid, info.lcid, info.major, info.minor)
dir_path_name = join
