...
fdesc = entry.desc
write(stream, "$(fdesc.memid)d : \"$(None)")
end
write(stream, "\t\t}")
end
...
write(stream, "$(("#\tdef " * methName * "(self" + BuildCallList(build, fdesc, entry.names, "defaultNamedOptArg", "defaultNamedNotOptArg", "defaultUnnamedArg", "pythoncom.Missing", true)))):")
...
write(stream, "$(n) is actually a property, but must be used as a method to correctly pass the arguments")
end
...
if generator.bBuildHidden || !(entry.hidden)
if GetResultName(entry)
write(stream, "$(key)\' returns object of type \'$(None)")
end
details = entry.desc
...
write(stream, "$(None)\" : (($(None), 0),($(pythoncom.DISPATCH_PROPERTYPUT)")
end
end
...
write(stream, "$(typename) for this class is \'$(None)")
for line in ret
write(stream, "$(line)")
end
...
write(stream, "$(t) - allow len(ob) to provide this")
for line in ret
write(stream, "$(line)")
end
...
write(stream, "$(s)(CoClassBaseClass): # A CoClass")
if doc && doc[2]
write(stream, "\t# $(doc[2])")
end
...
write(stream, "$(None)\' : $(item.python_name)")
end
end
write(stream, "}")
