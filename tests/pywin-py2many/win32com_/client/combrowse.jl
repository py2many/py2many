...
return [HLIHeadingCategory(), HLI_IEnumMoniker(EnumRunning(GetRunningObjectTable(pythoncom)), "Running Objects"), HLIHeadingRegisterdTypeLibs()]
end
...
catinf = CoCreateInstance(pythoncom, pythoncom.CLSID_StdComponentCategoriesMgr, nothing, pythoncom.CLSCTX_INPROC, pythoncom.IID_ICatInformation)
...
collected = []
helpPath = ""
key = RegOpenKey(win32api, win32con.HKEY_CLASSES_ROOT, "TypeLib\\%s\\%s" % (clsidstr, versionStr))
DoWaitCursor(win32ui, 1)
try
num = 0
while true
...
append(ret, MakeHLI(browser, refAttr[1], "Name=%s, Flags = %d" % (refAttr[1], flags)))
end
return ret
end
