...
regex
                    ...
                    Argument(arrayDecl::Int64, const_::Int64, good_interface_names, indirectionLevel::Int64, inout, name, raw_type, type_, unc_type, regex = compile(re, "/\\* \\[([^\\]]*.*?)] \\*/[ \\t](.*[* ]+)(\\w+)(\\[ *])?[\\),]")) =
...
if VERBOSE != 0
@printf("\t   Arg %s of type %s%s (%s)", (self.name, self.type, repeat("*",self.indirectionLevel), self.inout))
end
end
...
regex
                    ...
                    Method(args::Vector, callconv, good_interface_names, name, result, regex = compile(re, "virtual (/\\*.*?\\*/ )?(.*?) (.*?) (.*?)\\(\\w?")) =
