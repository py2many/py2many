...
function _study2(self::Parser)
    ...
    while i != 0
        @assert(p)
        q = p
        for nothing = goodlines[i]:goodlines[i+1]-1
            p = rfind(str, "\n", 0, p - 1) + 1
        end
        if _junkre(str, p)
            i = i - 1
...
function get_num_lines_in_stmt(self::Parser)::Union[int, auto]
    _study1(self)
