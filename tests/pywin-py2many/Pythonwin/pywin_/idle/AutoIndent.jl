...
mutable struct AutoIndent <: AbstractAutoIndent
    ...
    menudefs::Vector{Tuple}
    num_context_lines::Tuple
    ...
    AutoIndent(
        ...
        keydefs::Dict{String, Vector{String}} = Dict(
        ...
        num_context_lines::Tuple = (50, 500, 5000000),
        unix_keydefs::Dict{String, Vector{String}} = Dict(
            "<<indent-region>>" =>
                ["<Alt-bracketright>", "<Meta-bracketright>", "<Control-bracketright>"],
            "<<dedent-region>>" =>
                ["<Alt-bracketleft>", "<Meta-bracketleft>", "<Control-bracketleft>"],
        ...
        windows_keydefs::Dict{String, Vector{String}} = Dict(
...
function newline_and_indent_event(self::AutoIndent, event)::String
    ...
    try
        ...
        for context in self.num_context_lines
            ...
            set_str(y, rawtext)
            bod = find_good_parse_start(
                y,
                self.context_use_ps1,
                _build_char_in_string_func(self, startatindex),
            )
...
function toggle_tabs_event(self::AutoIndent, event)::String
    if askyesno(
        self.editwin,
        "Toggle tabs",
        ("Turn tabs " + ("on", "off")[self.usetabs+1]) * "?",
        parent = self.text,
    )
...
function classifyws(s, tabwidth)::Tuple
    raw = 0
    effective = 0
    for ch in s
        if ch == " "
