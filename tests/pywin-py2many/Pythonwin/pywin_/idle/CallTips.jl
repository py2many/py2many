...
mutable struct CallTips <: AbstractCallTips
    ...
    CallTips(
        ...
        menudefs::Vector = [],
        unix_keydefs::Dict = Dict(),
        windows_keydefs::Dict = Dict(),
    ) = begin
...
function check_calltip_cancel_event(self::CallTips, event)::String
    if self.calltip
        if compare(self.text, "insert", "<=", self.calltip_start) ||
           compare(self.text, "insert", ">", self.calltip_start + " lineend")
            _remove_calltip_window(self)
        end
    end
    return ""
