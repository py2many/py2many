...
mutable struct AutoExpand <: AbstractAutoExpand
    ...
    AutoExpand(
        state,
        text,
        keydefs::Dict{String, Vector{String}} = Dict("<<expand-word>>" => ["<Alt-slash>"]),
        menudefs::Vector{Tuple} = [("edit", [("E_xpand word", "<<expand-word>>")])],
        unix_keydefs::Dict{String, Vector{String}} = Dict(
            "<<expand-word>>" => ["<Meta-slash>"],
        ),
        wordchars::String = (string.ascii_letters + string.digits) + "_",
...
function getwords(self::AutoExpand)::Vector
    ...
    wbefore = findall(("\\b" + word) * "\\w+\\b", before)
    #Delete Unsupported
    del(before)
    after = get(self.text, "insert wordend", "end")
    wafter = findall(("\\b" + word) * "\\w+\\b", after)
    #Delete Unsupported
    del(after)
    if !(wbefore) && !(wafter)
        return []
...
function getprevword(self::AutoExpand)
    ...
    i = length(line)
    while i > 0 && line[i] in self.wordchars
        i = i - 1
    end
    return line[i+1:end]
end
