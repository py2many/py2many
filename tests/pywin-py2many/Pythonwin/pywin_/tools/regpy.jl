...
mutable struct RegEditPropertyPage <: AbstractRegEditPropertyPage
    ...
    RegEditPropertyPage(IDC_LISTVIEW::Int64 = 1000) = new(IDC_LISTVIEW)
end
function GetTemplate(self::RegEditPropertyPage)
    ...
    lvStyle =
        (
            (
                (
                    ((SS_STD | commctrl.LVS_EDITLABELS) | commctrl.LVS_REPORT) |
                    commctrl.LVS_AUTOARRANGE
                ) | commctrl.LVS_ALIGNLEFT
            ) | win32con.WS_BORDER
        ) | win32con.WS_TABSTOP
    append(template, ["SysListView32", "", self.IDC_LISTVIEW, (10, 10, 185, 100), lvStyle])
    return template
end
