...
_better_names =
    Dict("escape" => "esc", "return" => "enter", "back" => "pgup", "next" => "pgdn")
function _fillvkmap()
...
_fillvkmap()
function get_vk(chardesc)::Tuple
...
function parse_key_name(name)::Tuple
    name = name + "-"
    start = 0
    pos = 0
    max = length(name)
