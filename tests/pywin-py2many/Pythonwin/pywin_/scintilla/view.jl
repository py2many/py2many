...
mutable struct CScintillaView <: docview.CtrlView
    ...
    CScintillaView(doc) = begin
        docview.CtrlView.__init__(
            ...
            "Scintilla",
            (
                ((win32con.WS_CHILD | win32con.WS_VSCROLL) | win32con.WS_HSCROLL) |
...
function HookHandlers(self::CScintillaView)
    for (name, val) in event_commands
        bind(self.bindings, name, nothing, cid = val)
    end
...
function SaveTextFile(self::CScintillaView, filename, encoding = nothing)::Int64
    doc = GetDocument(self)
    _SaveTextToFile(doc, self, filename, encoding = encoding)
...
function _AutoComplete(self::CScintillaView)
    ...
    if ob != nothing
        try
            ...
            if hasattr(ob, "_oleobj_")
                try
                    for iTI = 0:GetTypeInfoCount(ob._oleobj_)-1
    ...
    if !(items)
        ...
        items = [
            word for word in keys(unique) if
            word[begin:2] != "__" || word[length(word)-2+1:end] != "__"
        ]
        try
...
function _GetClassInfoFromBrowser(self::CScintillaView, pos = -1)::Tuple
    minline = 0
...
function OnPrint(self::CScintillaView, dc, pInfo)
    ...
    nextPageStart =
        FormatRange(self, dc, self.starts[pageNum+1], self.starts[pageNum+2], rc, 1)
end
