...
function close(self::IDLEEditorWindow)
    ...
    try
        for ext in values(self.extensions)
            closer = getattr(ext, "close", nothing)
            if closer != nothing
                closer()
            end
        end
...
function IDLEExtension(self::IDLEEditorWindow, extension)
    ...
    events = [item for item in dir(klass) if item[length(item)-6+1:end] == "_event"]
    for event in events
        name = "<<%s>>" % (replace(event[begin:-6], "_", "-"),)
        bind(self.edit.bindings, name, getattr(ext, event))
    end
    return ext
end
...
function _NextTok(str, pos)::Tuple
    ...
    while pos < end_ && str[pos+1] in string.whitespace
        pos = pos + 1
    end
    if str[pos+1] in "+-"
    ...
    endPos = pos
    while endPos < end_ && str[endPos+1] in (string.digits + ".")
        endPos = endPos + 1
    end
    if pos != endPos
...
function TkIndexToOffset(bm, edit, marks)
    ...
    while true
        ...
        elseif word == "wordstart"
            while pos > 0 && SCIGetCharAt(edit, pos - 1) in wordchars
                pos = pos - 1
            end
        elseif word == "wordend"
            end_ = GetTextLength(edit)
            while pos < end_ && SCIGetCharAt(edit, pos) in wordchars
                pos = pos + 1
            end
...
function _fix_indexes(self::TkText, start, end_)::Tuple
    while start > 0 && (ord(SCIGetCharAt(self.edit, start)) & 192) == 128
        start -= 1
    end
    while end_ < GetTextLength(self.edit) &&
        (ord(SCIGetCharAt(self.edit, end_)) & 192) == 128
        end_ += 1
    end
    if start > 0 &&
       SCIGetCharAt(self.edit, start) == "\n" &&
       SCIGetCharAt(self.edit, start - 1) == "\r"
        start = start - 1
    end
    if end_ < GetTextLength(self.edit) &&
       SCIGetCharAt(self.edit, end_ - 1) == "\r" &&
       SCIGetCharAt(self.edit, end_) == "\n"
        end_ = end_ + 1
    end
    return (start, end_)
end
