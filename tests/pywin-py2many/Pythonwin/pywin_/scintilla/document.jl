...
crlf_bytes = Vector{UInt8}("\r\n")
lf_bytes = Vector{UInt8}("\n")
re_encoding_bytes = compile(re, Vector{UInt8}("coding[:=]\\s*([-\\w.]+)"))
...
function SaveFile(self::CScintillaDocument, fileName, encoding = nothing)
    ...
    ok = SaveTextFile(view, fileName, encoding = encoding)
    if ok
...
function HookViewNotifications(self::CScintillaDocument, view)
    ...
    HookNotify(
        parent,
        ViewNotifyDelegate(self, "OnBraceMatch"),
        scintillacon.SCN_CHECKBRACE,
    )
    HookNotify(
        parent,
        ViewNotifyDelegate(self, "OnMarginClick"),
...
function _ApplyOptionalToViews(self::CScintillaDocument, funcName)
    for view in GetAllViews(self)
        func = getattr(view, funcName, nothing)
        if func != nothing
            func(args...)
        end
    end
