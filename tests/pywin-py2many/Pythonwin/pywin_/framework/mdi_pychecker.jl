            ...
            elseif os.path.isfile(d)
                ...
                elseif d[begin:5] == "HKEY_"
                    ...
                    try
                        ...
                        win32ui.MessageBox(
                            "Can\'t interpret registry key value: %s" % keystr[1:end],
                        )
                    end
                else
...
function setInitParams(self::TheDocument, paramstr)
    ...
    if !(self.dirpattern)
        try
            ...
            self.dirpattern =
                abspath(os.path, dirname(os.path, GetPathName(GetDocument(editor))))
        catch exn
...
function _inactive_idleHandler(self::TheDocument, handler, count)::Int64
    ...
    if self.fndx < length(self.flist)
        ...
        if self.fpndx < length(self.fplist)
            self.flist =
                glob(glob, (self.dp[self.dpndx+1] + "\\") + self.fplist[self.fpndx+1])
        else
            self.fpndx = 0
...
function GetParams(self::TheDocument)::String
    return (
        (
            self.dirpattern * "\t" * self.filpattern * "\t" * self.greppattern * "\t" +
        ...
        if m
            if first
                ...
                cmnt = GetSimpleInput(
                    ...
                    addspecific && "  #\$pycheck_no=%(errtext)s" || "  #\$pycheck_no",
                )
                if !(cmnt)
                    return 0
                end
            ...
            if GetTextRange(view, pos - 1, pos) in ("\r", "\n")
                pos -= 1
            end
            SetSel(view, pos, pos)
            errtext = group(m, 3)
...
mutable struct TheParamsDialog <: AbstractTheParamsDialog
    ...
    TheParamsDialog(items) = begin
        tmp.append([
            ...
            (
                (
                    ((CS | win32con.LBS_MULTIPLESEL) | win32con.LBS_STANDARD) |
