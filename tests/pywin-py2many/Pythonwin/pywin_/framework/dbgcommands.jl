...
IdToBarNames = Dict(win32ui.IDC_DBG_STACK => ("Stack", 0), win32ui.IDC_DBG_BREAKPOINTS => ("Breakpoints", 0), win32ui.IDC_DBG_WATCH => ("Watch", 1))
mutable struct DebuggerCommandHandler <: AbstractDebuggerCommandHandler
OnDebuggerBar
OnUpdateDebuggerBar
end
function HookCommands(self::DebuggerCommandHandler)
commands = ((self.OnStep, nothing, win32ui.IDC_DBG_STEP), (self.OnStepOut, self.OnUpdateOnlyBreak, win32ui.IDC_DBG_STEPOUT), (self.OnStepOver, nothing, win32ui.IDC_DBG_STEPOVER), (self.OnGo, nothing, win32ui.IDC_DBG_GO), (self.OnClose, self.OnUpdateClose, win32ui.IDC_DBG_CLOSE), (self.OnAdd, self.OnUpdateAddBreakpoints, win32ui.IDC_DBG_ADD), (self.OnClearAll, self.OnUpdateClearAllBreakpoints, win32ui.IDC_DBG_CLEAR))
...
d = _GetDebugger(self)
if d === nothing
MarkerToggle(doc, lineNo, win32com_.gen_py.framework.editor.color.coloreditor.MARKER_BREAKPOINT)
else
...
MarkerDeleteAll(doc, win32com_.gen_py.framework.editor.color.coloreditor.MARKER_BREAKPOINT)
end
else
clear_all_breaks(d)
...
enabled = 1
lineNo = LineFromChar(view, GetSel(view)[1]) + 1
SetCheck(cmdui, MarkerAtLine(doc, lineNo, win32com_.gen_py.framework.editor.color.coloreditor.MARKER_BREAKPOINT) != 0)
end
