...
using win32com_.gen_py.framework.editor:
    GetEditorOption,
    SetEditorOption,
    GetEditorFontOption,
    SetEditorFontOption,
...
function DoConfigChange(self::SyntEditView)
    ...
    if bSmartTabs
        config(ext, usetabs = 1, tabwidth = 5, indentwidth = 4)
        ...
        config(ext, usetabs = usetabs, indentwidth = indentwidth, tabwidth = tabSize)
    else
        config(ext, usetabs = bUseTabs, tabwidth = tabSize, indentwidth = indentSize)
    end
...
function OnRClick(self::SyntEditView, params)::Int64
    ...
    AppendMenu(self, menu, flags = win32con.MF_SEPARATOR)
    ...
    AppendMenu(
        ...
        "&Fixed Font",
        "ViewFixedFont",
        checked = _GetColorizer(self).bUseFixed,
    )
    AppendMenu(self, menu, flags = win32con.MF_SEPARATOR)
...
function FoldExpandSecondLevelEvent(self::SyntEditView, event)::Int64
    ...
    for lineno = 0:GetLineCount(self)-1
...
function FoldExpandEvent(self::SyntEditView, event)::Int64
    ...
    DoWaitCursor(win32ui, 1)
    lineno = LineFromChar(self, GetSel(self)[1])
    if SCIGetFoldLevel(self, lineno) & scintillacon.SC_FOLDLEVELHEADERFLAG &&
       !SCIGetFoldExpanded(self, lineno)
        SCIToggleFold(self, lineno)
    end
    DoWaitCursor(win32ui, -1)
end
...
function CheckIDLEMenus(self::SyntEditTemplate, idle)
    ...
    for (text, event) in newitems
        ...
        if id != nothing
            keyname = get_key_binding(
                win32com_.gen_py.scintilla.view.configManager,
                event,
                ["editor"],
            )
            if keyname != nothing
...
function GetPythonPropertyPages(self::SyntEditTemplate)::Vector
    ...
    return append!(
        GetPythonPropertyPages(EditorTemplateBase, self),
        [ScintillaFormatPropertyPage(configui)],
    )
end
