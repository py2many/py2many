...
function OnSaveDocument(self::EditorDocumentBase, fileName)::Int64
    ...
    elseif self.bakFileType == BAK_DOT_BAK_TEMP_DIR
        bakFileName =
            ((GetTempPath(win32api) + "\\") + splitext(os.path, basename)[1]) * ".bak"
        ...
        let details = exn
            if details isa (UnicodeEncodeError, LookupError)
                ...
                if rc == win32con.IDYES
                    try
                        SaveFile(self, fileName, encoding = "latin-1")
                    catch exn
                        let details = exn
                            if details isa IOError
                                MessageBox(
...
function CheckExternalDocumentUpdated(self::EditorDocumentBase)
    ...
    changed =
        ...
        self.fileStat[1] != newstat[1] ||
        self.fileStat[7] != newstat[7] ||
        self.fileStat[9] != newstat[9] ||
        self.fileStat[10] != newstat[10]
    if changed
        question = nothing
        if IsModified(self)
            question =
