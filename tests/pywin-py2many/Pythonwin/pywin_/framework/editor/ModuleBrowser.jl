...
function TakeDefaultAction(self::HierListCLBRItem)
    if self.file
        JumpToDocument(
            win32com_.gen_py.framework.scriptutils,
            self.file,
            self.lineno,
            bScrollToTop = 1,
        )
...
mutable struct BrowserView <: AbstractBrowserView
    ...
    rootitem::Union[
        Union[
            Union[
                Union[
                    Union[HierListCLBRErrorRoot, HierListCLBRModule],
                    HierListCLBRErrorRoot,
                ],
                HierListCLBRErrorRoot,
            ],
            HierListCLBRModule,
...
function _MakeRoot(
    ...
    Union[
        Union[
            Union[Union[HierListCLBRErrorRoot, HierListCLBRModule], HierListCLBRErrorRoot],
            HierListCLBRErrorRoot,
        ],
        HierListCLBRModule,
    ],
...
function DestroyList(self::BrowserView)
    self.destroying = 1
    list = getattr(self, "list", nothing)
    self.list = nothing
    if list != nothing
        HierTerm(list)
    end
