...
function OnClose(self::MainFrame)
    try
        if win32com_.gen_py.debugger.currentDebugger != nothing &&
           win32com_.gen_py.debugger.currentDebugger.pumping
            try
...
function InitInstance(self::InteractivePythonApp)::Int64
    if "/nodde" ∉ append!([PROGRAM_FILE], ARGS) &&
       "/new" ∉ append!([PROGRAM_FILE], ARGS) &&
       "-nodde" ∉ append!([PROGRAM_FILE], ARGS) &&
       "-new" ∉ append!([PROGRAM_FILE], ARGS)
        if InitDDE(self) != 0
            return 1
        end
...
function LoadSystemModules(self::InteractivePythonApp)
    DoLoadModules(
        self,
        "win32com_.gen_py.framework.editor,win32com_.gen_py.framework.stdin",
    )
end
...
function OnFileLocate(self::InteractivePythonApp, id, code)
    ...
    if lower(lastLocateFileName[length(lastLocateFileName)-3+1:end]) == ".py"
