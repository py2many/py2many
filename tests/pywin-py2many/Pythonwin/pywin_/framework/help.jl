...
html_help_command_translators = Dict(win32con.HELP_CONTENTS => 1, win32con.HELP_CONTEXT => 15, win32con.HELP_FINDER => 1)
...
if ext == ".hlp"
WinHelp(win32api, GetSafeHwnd(GetMainFrame(win32ui)), fileName, helpCmd, helpArg)
elseif 0 && ext == ".chm"
global htmlhelp_handle
helpCmd = get(html_help_command_translators, helpCmd, helpCmd)
frame = 0
if htmlhelp_handle === nothing
htmlhelp_hwnd, htmlhelp_handle = HtmlHelp(win32help, frame, nothing, win32help.HH_INITIALIZE)
end
...
retList = []
try
key = RegOpenKey(win32api, root, BuildDefaultPythonKey(regutil) + "\\Help", 0, win32con.KEY_READ)
catch exn
 let exc = exn
...
EnableMenuItem(helpMenu, otherHelpMenuPos, win32con.MF_BYPOSITION | win32con.MF_GRAYED)
end
end
