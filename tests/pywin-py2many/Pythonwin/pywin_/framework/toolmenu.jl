...
defaultToolMenuItems = [
    ...
    (
        "Browse PythonPath",
        "from win32com_.gen_py.tools import browseProjects;browseProjects.Browse()",
    ),
    (
        "Edit Python Path",
        "from win32com_.gen_py.tools import regedit;regedit.EditRegistry()",
    ),
    ("COM Makepy utility", "from win32com_.client import makepy;makepy.main()"),
    ("COM Browser", "from win32com_.client import combrowse;combrowse.main()"),
    (
        "Trace Collector Debugging tool",
        "from win32com_.gen_py.tools import TraceCollector;TraceCollector.MakeOutputWindow()",
    ),
]
...
function SetToolsMenu(menu, menuPos = nothing)
    ...
    InsertMenu(
        menu,
        menuPos,
        ((win32con.MF_BYPOSITION | win32con.MF_ENABLED) | win32con.MF_STRING) |
        win32con.MF_POPUP,
        GetHandle(toolsMenu),
        "&Tools",
    )
