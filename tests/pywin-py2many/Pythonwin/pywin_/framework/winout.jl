...
abstract type AbstractWindowOutputViewScintilla <:
              Abstractwin32com_.gen_py.scintilla.view.CScintillaView end
...
function HandleSpecialLine(self::WindowOutputViewImpl)::Int64
    ...
    if line[begin:11] == "com_error: "
        try
            det = eval(strip(line[find(line, ":")+1+1:end]))
...
function RestoreKillBuffer(self::WindowOutputViewScintilla)
    @assert(length(self.template.killBuffer) in [0, 1])
...
function RTFWindowOutput()::WindowOutput
    ...
    return WindowOutput(args..., None = kw)
end
