...
RS_DEBUGGER_GO = 2
RS_DEBUGGER_PM = 3
debugging_options = split(
    "No debugging\nStep-through in the debugger\nRun in the debugger\nPost-Mortem of unhandled exceptions",
    "\n",
)
byte_cr = Vector{UInt8}("\r")
byte_lf = Vector{UInt8}("\n")
byte_crlf = Vector{UInt8}("\r\n")
...
function GetPackageModuleName(fileName)::Tuple
    ...
    if !IsOnPythonPath(path) != 0
        while length(path) > 3
            ...
            push!(modBits, modBit)
            if IsOnPythonPath(path) && modBit in sys.modules && exists(os.path, join) ||
               exists(os.path, join) ||
               exists(os.path, join)
                reverse(modBits)
...
function GetActiveFileName(bAutoSave = 1)
    ...
    try
        ...
        if bAutoSave && length(pathName) > 0 ||
           GetTitle(doc)[begin:8] == "Untitled" ||
           GetTitle(doc)[begin:6] == "Script"
            if IsModified(doc)
                try
...
function RunScript(
        ...
        if debuggingType == RS_DEBUGGER_GO && debugger != nothing
            ...
            if rd != nothing && length(rd.breaks) == 0
                ...
                rc = MessageBox(
                    win32ui,
                    msg,
                    LoadString(win32ui, win32ui.IDR_DEBUGGER),
    ...
    try
        if debuggingType == RS_DEBUGGER_STEP
            run(debugger, codeObject, __main__.__dict__, start_stepping = 1)
        elseif debuggingType == RS_DEBUGGER_GO
            run(debugger, codeObject, __main__.__dict__, start_stepping = 0)
        else
...
function ImportFile()::Int64
    ...
    for (key, mod) in collect(items(sys.modules))
        if getattr(mod, "__file__", nothing)
            fname = mod.__file__
            base, ext = splitext(os.path, fname)
            if lower(ext) in [".pyo", ".pyc"]
                ext = ".py"
            end
            fname = base + ext
    ...
    try
        ...
        SetStatusText(
            win32ui,
            (("Successfully " + what) * "ed module \'" + modName) *
...
function FindTabNanny()
    ...
    try
        ...
        if exn isa win32api.error
            println(
                "WARNING - The Python registry does not have an \'InstallPath\' setting",
            )
