...
if look > 2
look_str = (SCIGetCharAt(self.scintilla, look - 2) + SCIGetCharAt(self.scintilla, look - 1)) + SCIGetCharAt(self.scintilla, look)
...
write(sys.stderr, "PythonWin %s on %s%s.\n" % (sys.version, sys.platform, suffix))
write(sys.stderr, "Portions %s - see \'Help/About PythonWin\' for further copyright information.\n" % (win32ui.copyright,))
else
...
if rcfile
try
exec(compile(read(readline(rcfile)), rcfile, "exec", true), __main__.__dict__, __main__.__dict__)
catch exn
...
bNeedIndent = GetKeyState(win32api, win32con.VK_SHIFT) < 0 || GetKeyState(win32api, win32con.VK_CONTROL) < 0
if bNeedIndent
ReplaceSel(self, "\n")
else
SetSel(self, -2)
...
if lastActive != nothing && lastActive._obj_ === nothing || GetSafeHwnd(lastActive) == 0
lastActive = nothing
...
makeFrame
            ...
            CInteractivePython(makeDoc = nothing, makeFrame = nothing) = begin
                winout.WindowOutput.__init__(self, sectionProfile, sectionProfile, winout.flags.WQ_LINE, 1, nothing, makeDoc, makeFrame, InteractiveView)
Create()
...
CreateWindow(bar, GetMainFrame(win32ui), creator, "Interactive Window", ID_DOCKED_INTERACTIVE_CONTROLBAR)
