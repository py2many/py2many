...
tracebackHeader = Vector{UInt8}("Traceback (")
...
function ColorizeInteractiveCode(self::InteractiveFormatter, cdoc, styleStart, stylePyStart)
    ...
    while i < lengthDoc
        ...
        elseif state in [STYLE_INTERACTIVE_OUTPUT]
            if ch in "\r\n"
                ColorSeg(self, startSeg, i - 1, state)
                startSeg = i
                state = STYLE_INTERACTIVE_EOL
        ...
        elseif state == STYLE_INTERACTIVE_BANNER
            ...
            end_ = startSeg
            while end_ < lengthDoc && cdoc[end_] ∉ Vector{UInt8}("\r\n")
                end_ = end_ + 1
            end
...
function Colorize(self::InteractiveFormatter, start = 0, end_ = -1)
    stringVal = GetTextRange(self.scintilla, start, end_, decode = false)
    styleStart = nothing
    stylePyStart = nothing
    if start > 1
        look = start - 1
        while look && SCIGetCharAt(self.scintilla, look) in "\n\r"
            look = look - 1
        end
...
function Init(self::InteractiveCore)
    ...
    if rcfile
        try
            exec(
                compile(read(readline(rcfile)), rcfile, "exec", dont_inherit = true),
                __main__.__dict__,
                __main__.__dict__,
            )
        catch exn
...
function EnsureNoPrompt(self::InteractiveCore)
    ...
    lastLineNo = GetLineCount(self) - 1
    line = DoGetLine(self, lastLineNo)
    if !(line) || line in [sys.ps1, sys.ps2]
...
function ProcessEnterEvent(self::InteractiveCore, event)::Int64
    ...
    if bNeedIndent
        ...
        pos = 0
        indent = ""
        while length(curLine) > pos && curLine[pos+1] in string.whitespace
