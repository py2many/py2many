...
# Removed multiprocessing and changed prints to list appends
...
def main(requested_max_depth, min_depth=4):
    # Append messages to list instead of printing
    io_lst: list[str] = []

    max_depth = max(min_depth + 2, requested_max_depth)
    stretch_depth = max_depth + 1

    io_lst.append(f'stretch tree of depth {stretch_depth}'
    ...
    for test_depth in range(min_depth, stretch_depth, 2):
        ...
        io_lst.append(f'{tree_count}\t trees of depth {test_depth}'
                      f'\t check: {check_sum}')

    io_lst.append(f'long lived tree of depth {max_depth}'
